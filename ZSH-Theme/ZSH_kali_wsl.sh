...
select_terminal() {
    if is_wsl; then
        echo "WSL detected. Using default WSL terminal configuration."
        TERMINAL_TYPE="wsl"
    else
...
configure_terminal_font() {
    if is_wsl; then
        echo "WSL detected. Please manually install JetBrainsMono Nerd Font in Windows."
        echo "Download from: https://github.com/ryanoasis/nerd-fonts/releases/download/v3.1.1/JetBrainsMono.zip"
        echo "Then install in Windows and configure your terminal to use it."
        return
    fi
...
if is_wsl; then
    echo "WSL environment detected. Configuring for WSL compatibility..."
fi
...
# Kitty installation only for non-WSL
if ! is_wsl; then
...
EOL
    ...
    # For WSL, skip kitty installation
    select_terminal
fi
...
# Nerd Fonts installation - skip download for WSL
if ! is_wsl; then
    ...
    echo "WSL detected: Skipping Nerd Font installation on Linux side."
    echo "Please install JetBrainsMono Nerd Font in Windows and configure your terminal."
fi

# Configure terminal font if not using Kitty and not in WSL
...
if is_wsl; then
    ...
    if [ "$ARCH" = "amd64" ]; then
        ...
        echo "Architecture $ARCH detected. Please install lsd manually for your architecture."
    fi
else
    TEMP_DEB="$(mktemp)"
    wget -O "$TEMP_DEB" https://github.com/Peltoche/lsd/releases/download/0.23.1/lsd_0.23.1_amd64.deb
...
# Create .zshrc with WSL-specific configurations if needed
cat > ~/.zshrc << 'EOL'
...
if grep -q microsoft /proc/version; then
  ...
  # Open files with default Windows application
  open() {
    if [ $# -eq 0 ]; then
      explorer.exe .
    else
      for file in "$@"; do
        if [ -d "$file" ]; then
          ...
          cmd.exe /c start "" "$(wslpath -w "$file")" > /dev/null 2>&1
        fi
      done
    fi
  }
...
# Create a README file with additional WSL setup instructions
if is_wsl; then
    cat > ~/WSL_SETUP_INSTRUCTIONS.md << 'EOL'
# Additional WSL Setup Instructions

## Font Installation
1. Download JetBrainsMono Nerd Font from: https://github.com/ryanoasis/nerd-fonts/releases/download/v3.1.1/JetBrainsMono.zip
...
2. Click on the dropdown arrow and select "Settings"
3. Find your Kali Linux profile
4. Change the "Font face" to "JetBrainsMono Nerd Font"
5. Save the settings
...
The script has set up aliases for clipboard integration:
...
- `code` - Open VS Code
- `open` - Open files with default Windows applications

## Path Conversion
Use the `wslpath` function to convert between WSL and Windows paths:
- `wslpath -w /path/in/wsl` - Convert WSL path to Windows path
- `wslpath -u C:\\path\\in\\windows` - Convert Windows path to WSL path
EOL

    echo "WSL-specific setup complete. Please check ~/WSL_SETUP_INSTRUCTIONS.md for additional setup steps."
fi
